...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnDept30EmpsOrderedBySalaryDesc()
    {
        ...
        List<Emp> result = emps.Where(e => e.DeptNo == 30).ToList().OrderByDescending(e => e.Sal).ToList(); 
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var grades = Database.GetSalgrades();
         ...
         var result = emps.Where(e => grades.Any(s=>e.Sal >= s.Losal && e.Sal <= s.Hisal))
             .Select(e=> new {e.EName, Grade = grades
                 .FirstOrDefault(s => e.Sal >= s.Losal && e.Sal <= s.Hisal)?.Grade}).ToList();
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        var emps = Database.GetEmps();
         ...
         var result = emps.GroupBy(e => e.DeptNo).Select(e => new {DeptNo = e.Key, AvgSal = emps.Average(e => e.Sal)}).ToList(); 
        //
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        var emps = Database.GetEmps();
         ...
         var result = emps.Where(e=>e.Sal > emps.Where(em=>em.DeptNo==e.DeptNo).Average(e => e.Sal)).Select(e => e.EName).ToList();
        //
